...
class EventRepositoryProtocol(typing.Protocol):
    ...
    async def get_all(self, data_from: typing.Optional[datetime], limit: int, offset: int) -> typing.Tuple[
        int, list]: ...
...
class EventsProviderClientProtocol(typing.Protocol):
    ...
    async def register(self, event_id: uuid.UUID, first_name: str, last_name: str, seat: str) -> str: ...


class SyncRepositoryProtocol(typing.Protocol):
    async def get_last_cursor(self) -> str: ...

    async def update_sync_info(self, last_changed_at: datetime, status: str) -> None: ...
